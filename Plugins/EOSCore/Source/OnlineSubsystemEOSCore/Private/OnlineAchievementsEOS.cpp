...
{
	int32 LocalUserId = EOSSubsystem->m_IdentityInterfacePtr->GetLocalUserNumFromUniqueNetId(PlayerId);
	if (LocalUserId < 0)
	{
	...
	Options.UserId = EOSSubsystem->m_IdentityInterfacePtr->GetLocalProductUserId(LocalUserId);

	FQueryProgressCallback* CallbackObj = new FQueryProgressCallback();
	CallbackObj->m_CallbackLambda = [this, LambaPlayerId = FUniqueNetIdEOS(PlayerId), OnComplete = FOnQueryAchievementsCompleteDelegate(Delegate)](const EOS_Achievements_OnQueryPlayerAchievementsCompleteCallbackInfo* Data)
	{
		...
		if (bWasSuccessful)
		{
			...
			int32 LocalUserNum = EOSSubsystem->m_IdentityInterfacePtr->GetLocalUserNumFromUniqueNetId(LambaPlayerId);
			EOS_ProductUserId UserId = EOSSubsystem->m_IdentityInterfacePtr->GetLocalProductUserId(LocalUserNum);
			...
			uint32 Count = EOS_Achievements_GetPlayerAchievementCount(EOSSubsystem->m_AchievementsHandle, &CountOptions);
...
void FOnlineAchievementsEOS::QueryAchievementDescriptions(const FUniqueNetId& PlayerId, const FOnQueryAchievementsCompleteDelegate& Delegate)
{
	...
	CallbackObj->m_CallbackLambda = [this, LambaPlayerId = FUniqueNetIdEOS(PlayerId), OnComplete = FOnQueryAchievementsCompleteDelegate(Delegate)](const EOS_Achievements_OnQueryDefinitionsCompleteCallbackInfo* Data)
	{
		bool bWasSuccessful = Data->ResultCode == EOS_EResult::EOS_Success;
		if (bWasSuccessful)
		{
			...
			uint32 Count = EOS_Achievements_GetAchievementDefinitionCount(EOSSubsystem->m_AchievementsHandle, &CountOptions);
