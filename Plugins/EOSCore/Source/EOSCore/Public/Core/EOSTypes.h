...
USTRUCT(BlueprintType, meta = (HasNativeBreak = "EOSCore.EOSCoreLibrary.BreakLobbyIdStruct", HasNativeMake = "EOSCore.EOSCoreLibrary.MakeLobbyIdStruct"))
struct FEOSLobbyId
{
	GENERATED_BODY()
public:
	FEOSLobbyId() {}
	FEOSLobbyId(FString lobbyId)
	{
		FCStringAnsi::Strncpy(m_LobbyId, TCHAR_TO_UTF8(*lobbyId), 512);
	}
public:
	FString ToString() const
	{
		return FString(UTF8_TO_TCHAR(m_LobbyId));
	}
public:
...
public:
	FAccessToken()  {}
	FAccessToken(FString token)
	{
		FCStringAnsi::Strncpy(const_cast<char*>(m_AccessToken), TCHAR_TO_UTF8(*token), 2048);
	}

	FString ToString() const
	{
		return FString(UTF8_TO_TCHAR(m_AccessToken));
	}
...
public:
	FLobbyInviteId() { }
	FLobbyInviteId(FString inviteId)
	{
		FCStringAnsi::Strncpy(m_InviteId, TCHAR_TO_UTF8(*inviteId), 512);
	}
public:
	FString ToString() const
	{
		return FString(UTF8_TO_TCHAR(m_InviteId));
	}
public:
	operator const char*() const { return m_InviteId; }
private:
	char m_InviteId[512];
};
