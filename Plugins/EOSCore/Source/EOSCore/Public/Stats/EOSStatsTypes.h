...
public:
	...
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = "Stats")
		FEOSProductUserId LocalUserId;
	/** If not EOS_STATS_TIME_UNDEFINED then this is the POSIX timestamp for start timeOptional). */
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = "Stats")
		FString StartTime;
...
//		Operations
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ //
struct FQueryStatsOperation : public EOS_Stats_QueryStatsOptions
{
public:
	FQueryStatsOperation(int32 NumStats, EOS_ProductUserId localUserId, EOS_ProductUserId targetUserId, int64_t startTime, int64_t endTime)
		: EOS_Stats_QueryStatsOptions()
	{
		PointerArray.AddZeroed(NumStats);
		...
		StatNamesCount = NumStats;
		TargetUserId = localUserId;
	}

	~FQueryStatsOperation()
	{
		for (int32 i = 0; i < PointerArray.Num(); i++)
		{
...
struct FIngestStatOperation : public EOS_Stats_IngestStatOptions
{
public:
	FIngestStatOperation(int32 NumStats, EOS_ProductUserId localUserId, EOS_ProductUserId targetUserId)
		: EOS_Stats_IngestStatOptions()
	{
		...
		for (int32 i = 0; i < PointerArray.Num(); i++)
		{
			PointerArray[i].ApiVersion = EOS_STATS_INGESTDATA_API_LATEST;
			PointerArray[i].StatName = new char[256];
		}

		ApiVersion = EOS_STATS_INGESTSTAT_API_LATEST;
		LocalUserId = localUserId;
		StatsCount = NumStats;
		Stats = PointerArray.GetData();
		TargetUserId = targetUserId;
	}

	~FIngestStatOperation()
	{
		for (int32 i = 0; i < PointerArray.Num(); i++)
		{
			delete[] PointerArray[i].StatName;
		}
	}
public:
	TArray<EOS_Stats_IngestData> PointerArray;
};